...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    private void sink(int index) {
        ...
        if (right > size){
            if ((contents[left].priority() < contents[index].priority())){
                swap(left, index);
                return;
            }
            return;
        }
        if ((contents[index].priority() > contents[right].priority()) || (contents[index].priority() > contents[left].priority())) {
